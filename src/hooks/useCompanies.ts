...
import type { SearchFilters, SearchResponse, CompanyWithRoles } from '../types';
...
export function useCompanies(): UseCompaniesReturn {
  ...
  const [currentFilters, setCurrentFilters] = useState<SearchFilters>({
    ...
    // Standardverdier for organisasjonsform
    organisasjonsform: ['AS'],
    // Standardverdier for næringskoder
    inkluderNaeringskoder: ['47', '56', '96', '43', '62'],
    ekskluderNaeringskoder: ['64', '70.10', '68.20', '46'],
  });
  ...
  const search = useCallback(async (filters: SearchFilters) => {
    ...
    try {
      ...
      // Always send næringskoder arrays (even if empty) so backend knows to apply filters
  ...
  // Initial search on mount with default filters (last 30 days)
  useEffect(() => {
